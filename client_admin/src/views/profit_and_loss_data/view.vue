	...
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
				...
				<el-form-item label="报表标题" prop="report_title">
							  ...
							  v-if="user_group === '管理员' || (form['profit_and_loss_data_id'] && $check_field('set','report_title')) || (!form['profit_and_loss_data_id'] && $check_field('add','report_title'))" :disabled="disabledObj['report_title_isDisabled']"></el-input>
				...
				<el-form-item label="报表日期" prop="report_date">
								<el-date-picker :disabled="disabledObj['report_date_isDisabled']" v-if="user_group === '管理员' || (form['profit_and_loss_data_id'] && $check_field('set','report_date')) || (!form['profit_and_loss_data_id'] && $check_field('add','report_date'))" id="report_date"
				...
				<el-form-item label="盈利金额" prop="profit_amount">
							  ...
							  v-if="user_group === '管理员' || (form['profit_and_loss_data_id'] && $check_field('set','profit_amount')) || (!form['profit_and_loss_data_id'] && $check_field('add','profit_amount'))" :disabled="disabledObj['profit_amount_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','profit_amount')">{{form['profit_amount']}}</div>
											</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','loss_amount') || $check_field('add','loss_amount') || $check_field('set','loss_amount')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="亏损金额" prop="loss_amount">
												<el-input id="loss_amount" v-model="form['loss_amount']" placeholder="请输入亏损金额"
							  v-if="user_group === '管理员' || (form['profit_and_loss_data_id'] && $check_field('set','loss_amount')) || (!form['profit_and_loss_data_id'] && $check_field('add','loss_amount'))" :disabled="disabledObj['loss_amount_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','loss_amount')">{{form['loss_amount']}}</div>
											</el-form-item>
			</el-col>
				...
				<el-form-item label="记录人员" prop="recording_personnel">
							  ...
							  v-if="user_group === '管理员' || (form['profit_and_loss_data_id'] && $check_field('set','recording_personnel')) || (!form['profit_and_loss_data_id'] && $check_field('add','recording_personnel'))" :disabled="disabledObj['recording_personnel_isDisabled']"></el-input>
				...
				<el-form-item label="备注信息" prop="remarks">
						...
						v-if="user_group === '管理员' || (form['profit_and_loss_data_id'] && $check_field('set','remarks')) || (!form['profit_and_loss_data_id'] && $check_field('add','remarks'))" :disabled="disabledObj['remarks_isDisabled']"></el-input>
