<template>
	<view class="diy_details diy_div_cancellation_of_order">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					...
					<view v-if="$check_field('get','reason_for_unsubscription')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>退订原因:</span>
						</view>
							<view class="diy_field diy_desc">
							<text>
								{{ obj["reason_for_unsubscription"] }}
							</text>
						</view>
						</view>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/cancellation_of_order/get_obj", {
					cancellation_of_order_id:this.query.cancellation_of_order_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["cancellation_of_order_id"] !==0) {
...
<style scoped>
	.div_cancellation_of_order_details {}
