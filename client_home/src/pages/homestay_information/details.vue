<template>
	<view class="page_homestay_information diy_detail diy_detail--new" id="homestay_information_details">
		<view class="warp">
			<view class="container-fluid">
				<view class="row">
					<view v-if="$check_field('get','name_of_homestay')" class="col-12 col-6">
						<view class="view">
					...
					<view v-if="$check_field('get','landlord_user')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>房东用户</span>
							</view>
					...
					<view v-if="$check_field('get','region_name')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>地区名称</span>
							</view>
					...
					<view v-if="$check_field('get','homestay_address')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>民宿地址</span>
							</view>
					...
					<view v-if="$check_field('get','room_number')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>房间号</span>
							</view>
					...
					<view v-if="$check_field('get','check_in_price')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>入住价格</span>
							</view>
					...
					<view v-if="$check_field('get','detailed_introduction')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>详情介绍</span>
							</view>
		...
		<view class="warp">
			<view class="container-fluid container-fluid-comment--new">
				<view class="row">
					<view class="col">
									<button type="button" class="btn btn-primary" v-if="$check_action('/order_management/edit','add')" @click="to_form('/pages/order_management/edit')">
							<span>预订</span>
						</button>
...
<!--						<button type="button" class="btn btn-primary" @click="cancel()">返回</button>-->
						...
						<navigator class="btn link"
							:url="'/pages/comment/edit?source_table=homestay_information&source_field=homestay_information_id&source_id=' + obj['homestay_information_id']">
							评论
						</navigator>
	...
	export default {
		...
		methods: {
			...
			get_praise(obj) {
				...
				this.$get("~/api/praise/count?", {
					...
					source_id: obj["homestay_information_id"]
				}, (res) => {
        ...
        _this.obj.praise_len = parseInt(_this.obj.praise_len)
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/homestay_information/set?homestay_information_id=' + _this.obj["homestay_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					this.state_collect = true;
								query.title = this.obj.name_of_homestay
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/homestay_information/set?homestay_information_id=' + obj["homestay_information_id"], {
					hits
				}, (res) => {
					if(res.result){
