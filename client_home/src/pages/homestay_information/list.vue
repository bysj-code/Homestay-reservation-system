<template>
	<view class="page_homestay_information diy_list diy_list_page div_list_tml" id="homestay_information_list">
		<!-- 筛选模块(开始) -->
		<view>
			<view class="container">
				<view>
					<view>
						<view class="">
							<!-- 搜索栏 -->
													<uni-search-bar placeholder="搜索民宿名称" @confirm="search" @cancel="cancel" cancelText="取消"
											@input="input($event, 'name_of_homestay')">
				...
				<view>
					<view>
						<view class="tab_view">
							<!-- 筛选栏 -->
																		<uni-section title="地区名称" type="line">
							<uni-data-select
									@change="change_region_name"
									v-model="query['region_name']"
									:localdata="list_region_name"
							></uni-data-select>
							</uni-section>
																					<uni-section title="房间类型" type="line">
							<uni-data-select
			...
			<view class="container">
				<view v-for="(o,i) in list" class="row box_style">
					...
					<view class="uni-padding-wrap uni-common-mt">
						<navigator :url="'./details?homestay_information_id=' + o['homestay_information_id']" class="btn btn-primary">查看详情</navigator>
...
<script>
			...
			return {
				url_get_list: "~/api/homestay_information/get_list?like=0",
					...
					list_user_landlord_user: [],
								// 地区名称选项列表
				list_region_name: [{value:"全部",text:"全部"}],
											// 房间类型选项列表
	 ...
	 */
	get_praise(list){
		this.$get("~/api/praise/count_group?groupby=source_id&source_table=homestay_information", {}, (res) => {
			if(res.result){
				res.result.map((o)=>{
					for(var i = 0;i < list.length; i++){
						var oj = list[i];
						if(oj["homestay_information_id"] === o["source_id"]){
