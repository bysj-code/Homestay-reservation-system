<template>
  <view class="page_homestay_information diy_table diy_table_tml" id="homestay_information_table">
    <!-- 筛选模块(开始) -->
    <view>
          ...
          <view>
            <view class="">
              ...
              <uni-forms :modelValue="query">
                  <uni-forms-item label="民宿名称" name="name_of_homestay">
                      <uni-easyinput type="text" v-model="query.name_of_homestay" placeholder="民宿名称" />
                    </uni-forms-item>
                      <uni-forms-item label="地区名称" name="region_name">
                  <uni-data-select placeholder="请选择"
            v-model="query['region_name']"
            :localdata="list_region_name"
        ></uni-data-select>
                         </uni-forms-item>
                        <uni-forms-item label="房间类型" name="room_type">
                  <uni-data-select placeholder="请选择"
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/homestay_information/view?" v-show="user_group == '管理员' || $check_action('/homestay_information/table','add') || $check_action('/homestay_information/view','add')">添加</navigator>
	...
	<view class="warp" >
		<view class="container">
			<view v-for="(o,i) in list" class="row box_style">
									<view class="col" v-if="1 && $check_field('get','name_of_homestay')">
						<view class="view">
							<view class="diy_title" v-if="true">
								...
								<span>
									...
									<view class="col" v-if="0 && $check_field('get','landlord_user')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>房东用户</span>
							</view>
								...
								<span>
									{{ get_user_landlord_user(o['landlord_user']) }}
								</span>
								</view>
													</view>
					</view>
									<view class="col" v-if="0 && $check_field('get','region_name')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>地区名称</span>
							</view>
								...
								<span>
									...
									<view class="col" v-if="0 && $check_field('get','homestay_address')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>民宿地址</span>
							</view>
								...
								<span>
									...
									<view class="col" v-if="1 && $check_field('get','room_number')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>房间号</span>
							</view>
						...
						<view class="view">
							<view class="diy_title" v-if=" false">
								<span>封面图片</span>
									...
									<view class="col" v-if="1 && $check_field('get','check_in_price')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>入住价格</span>
							</view>
				...
				<view class="col">
					<view class="view">
						<view class="create_time_block diy_filed diy_time">
                          ...
                          <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/homestay_information/table','del') || $check_action('/homestay_information/view','del')">删除</button>
                        </view>
					</view>
				</view>
				<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/homestay_information/view?' + field + '=' + o[field]"
						v-show="user_group == '管理员' || $check_action('/homestay_information/table','set') || $check_action('/homestay_information/view','set') || $check_action('/homestay_information/view','get')" class="btn btn-primary">详情</navigator>
					                    																		<!--跨表按钮-->
															<a type="button" size="mini" class="btn btn-primary" v-if="$check_action('/order_management/view','add')" @click="to_form('/pages/order_management/view',o)">
									<span>预订</span>
								</a>
																						</view>
			</view>
		...
		data() {
			return {
                ...
                list_user_landlord_user: [],
                        // 地区名称列表
                list_region_name: [{value:"",text:"全部"}],
                                            // 房间类型列表
