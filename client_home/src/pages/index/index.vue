<template>
  <view id="home" class="home--new">
    ...
    <Card
      class="list_diy"
      title="民宿信息推荐"
      url="/pages/homestay_information/list"
      :list="list_article"
      v-if="$check_action('/homestay_information/list', 'get')"
    >
	  <list_homestay_information :list="list_homestay_information"></list_homestay_information>
  ...
  methods: {
    ...
    toToggle() {
      this.isAdmin = !this.isAdmin;
		...
		*  获取民宿信息
		*/
		get_homestay_information() {
			let url = "~/api/homestay_information/get_list?";
									
			this.$get(url, {
											}, (json) => {
					if (json.result && json.result.list) {
					console.log("民宿信息" ,json.result.list);
					this.list_homestay_information = json.result.list;
											this.list_homestay_information.map((o)=>{
							o["praise_len"];
						});
						this.get_praise(this.list_homestay_information ,"homestay_information" ,"37880");
									}
			});
		},
