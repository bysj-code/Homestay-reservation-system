<template>
  <view class="page_order_management diy_table diy_table_tml" id="order_management_table">
    <!-- 筛选模块(开始) -->
    <view>
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/order_management/view?" v-show="user_group == '管理员' || $check_action('/order_management/table','add') || $check_action('/order_management/view','add')">添加</navigator>
                            ...
                            <span>订单编号</span>
								...
								<text>
									{{ o["order_number"] }}
													...
													<view v-if="1 && $check_field('get','name_of_homestay')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>民宿名称</span>
                          </view>
								...
								<text>
									{{ o["name_of_homestay"] }}
													...
													<view v-if="1 && $check_field('get','room_number')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>房间号</span>
                          </view>
                            ...
                            <span>房间房型</span>
								...
								<text>
									{{ o["room_type"] }}
													...
													<view v-if="1 && $check_field('get','check_in_price')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>入住价格</span>
                          </view>
								...
								<text>
									{{ o["check_in_price"] }}
													...
													<view v-if="0 && $check_field('get','booking_users')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>预订用户</span>
                          </view>
								...
								<text>
									{{ get_user_booking_users(o['booking_users']) }}
								</text>
							</view>
										</view>
					</view>
                            ...
                            <span>身份证号</span>
								...
								<text>
									{{ o["id_number"] }}
													...
													<view v-if="0 && $check_field('get','number_of_check_in_days')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>入住天数</span>
                          </view>
											<view class="diy_field diy_text text">
								<text>
									{{ o["number_of_check_in_days"] }}
								</text>
							</view>
										</view>
					</view>
													<view v-if="0 && $check_field('get','total_price')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>合计总价</span>
                          </view>
                  ...
                  <view class="col">
                    <view class="view">
                      <view class="create_time_block diy_filed diy_time">
                        ...
                        <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/order_management/table','del') || $check_action('/order_management/view','del')">删除</button>
                      </view>
                    </view>
                  </view>
					<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/order_management/view?' + field + '=' + o[field]"
                               v-show="user_group == '管理员' || $check_action('/order_management/table','set') || $check_action('/order_management/view','set') || $check_action('/order_management/view','get')" class="btn btn-primary">详情</navigator>
	                    <a @click="goto_pay(o)" size="mini"  v-if="o.pay_state==='未支付' && $check_pay('/order_management/table')" class='btn btn-primary'>支付</a>
	                                    <!--跨表按钮-->
		...
		methods: {
          ...
          reset: function reset() {
            ...
            if(user_group != "管理员"){
                  let sqlwhere = "(";
                                                                                                              if(user_group=="普通用户"){
                  sqlwhere+= "booking_users = " + this.user.user_id + " or ";
              }
          ...
          submit_pay(){
            ...
            this.$post(url,param,(res)=>{
              if(res.result){
                _this.$toast("支付成功");
                setTimeout(()=>{
                  _this.$nav("/pages/order_management/table");
                }, 700);
              }else if(res.error){
