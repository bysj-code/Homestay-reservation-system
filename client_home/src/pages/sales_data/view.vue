          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','report_title') || ($check_field('add','report_title') || $check_field('set','report_title'))" label="报表标题" name="report_title">
                            <uni-easyinput type="text" v-model="form['report_title']" v-if="user_group === '管理员' || (form['sales_data_id'] && $check_field('set','report_title')) || (!form['sales_data_id'] && $check_field('add','report_title'))" :disabled="disabledObj['report_title_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','report_date') || ($check_field('add','report_date') || $check_field('set','report_date'))" label="报表日期" name="report_date">
                    <uni-datetime-picker v-if="user_group === '管理员' || (form['sales_data_id'] && $check_field('set','report_date')) || (!form['sales_data_id'] && $check_field('add','report_date'))" v-model="form['report_date']" type="date" :disabled="disabledObj['report_date_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','report_date')">
                  {{ form['report_date'] }}
                </text>
                  </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','sales_volumes') || ($check_field('add','sales_volumes') || $check_field('set','sales_volumes'))" label="销售数量" name="sales_volumes">
                            <uni-easyinput type="text" v-model="form['sales_volumes']" v-if="user_group === '管理员' || (form['sales_data_id'] && $check_field('set','sales_volumes')) || (!form['sales_data_id'] && $check_field('add','sales_volumes'))" :disabled="disabledObj['sales_volumes_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','sales_volumes')">
                  {{ form['sales_volumes'] }}
                </text>
                          </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','recording_personnel') || ($check_field('add','recording_personnel') || $check_field('set','recording_personnel'))" label="记录人员" name="recording_personnel">
                            <uni-easyinput type="text" v-model="form['recording_personnel']" v-if="user_group === '管理员' || (form['sales_data_id'] && $check_field('set','recording_personnel')) || (!form['sales_data_id'] && $check_field('add','recording_personnel'))" :disabled="disabledObj['recording_personnel_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','remarks') || ($check_field('add','remarks') || $check_field('set','remarks'))" label="备注信息" name="remarks">
                    <uni-easyinput type="textarea" v-model="form['remarks']" v-if="user_group === '管理员' || (form['sales_data_id'] && $check_field('set','remarks')) || (!form['sales_data_id'] && $check_field('add','remarks'))" :disabled="disabledObj['remarks_isDisabled']" />
                <!-- 仅查看 -->
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/sales_data/upload?'),
